...
b = 0.5
latt = 5.49
Ceref = np.array([[a, 0, b], [0, -a, b], [-a, 0, b], [0, a, b],
                 [a, 0, -b], [0, -a, -b], [-a, 0, -b], [0, a, -b]])*latt
Ceref_n = Ceref
a = np.sqrt(2.)/4.
...
def main():
  # initialization
  args = Namespace(perspecies=True, poscar='',
                   write_dos0=False, write_pdos=True, zoomin=None)
  atoms = atom_data()
  dos = dos_data()
  ...
  for i in range(atoms.natom):
    ...
    if ((atoms.constraints[i, 0] == 'T') or (atoms.constraints[i, 0] == 'F')):
      if (atoms.species[i] == 1):
        ...
        return
      least_missing_id = -1
      least_missing_value = np.max([len(P[ref][:, 0])
