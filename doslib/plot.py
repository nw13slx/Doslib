...
class plot:
    ...
    def pdos(self):
      ...
      # plot each orbital
        if any(x != -1 for x in dos.par_element):
            ...
            for i in range(4):
                if dos.par_orbital[i] is not None:
                    ...
                    y += [None, None]
                if atom.symbol:
                    line_label += [atom.symbol[dos.par_element[i]
                                               ]+"_"+par_symbol[i], None]
                else:
                    line_label += ["atomtype=" +
                                   str(dos.par_element[i])+" "+par_symbol[i]+"_orbital", None]
            simple("pdf_orbital", dos.Xenergy, y, loc_down,
                   loc_up, line_color, line_label)
        ...
        if (dos.par_element[2] >= 0):
            ...
            line_color = ['r', 'r', 'b', 'b']
            y = [dos.d_t2g[:, 0], dos.d_t2g[:, 1],
                 dos.d_eg[:, 0], dos.d_eg[:, 1]]
            simple("d_decompose", dos.Xenergy, y, loc_down,
                   loc_up, line_color, line_label)
            del y, line_label, line_color
            ...
            for i in range(atom.ntype):
                if dos.spin:
                    ...
                    if atom.symbol:
                        ...
                        line_label += ["species "+str(i)]
            simple("perspecies", dos.Xenergy, y, loc_down,
                   loc_up, line_color, line_label)
            del y, line_label, line_color
    ...
    def tot_dos(self):
        ...
        line_color = ['r', 'b']
        self.simple("DOS-tot", self.dos.Xenergy, y, self.dos.loc_down,
                    self.dos.loc_up, line_color, line_label)
        del y, line_label, line_color

    def simple(self, name, x, y, l_d, l_u, line_color, line_label):
        print "plotting file", name+".png"
        fig = plt.figure(0, figsize=(3.34, 2.5), dpi=300,
    ...
    def dos0_extra(self):
        ...
        if dos.spin:
            plt.fill_between(ef[l_d:fermiN], 0,
                             dos0[l_d:fermiN, 0], facecolor='red')
            plt.fill_between(ef[l_d:fermiN], 0,
                             dos0[l_d:fermiN, 1], facecolor='blue')
        else:
            plt.fill_between(ef[l_d:fermiN], 0,
                             dos0[l_d:fermiN], facecolor='red')
