...
class analysis:
    ...
    def peak_finder(self, center, span):
        ...
        sigma = (np.square(np.arange(win_span)-peak)
                 ).dot(ana_dos)/np.sum(ana_dos)
        sigma = np.sqrt(sigma)
        dx = dos.Xenergy[1]-x0
    ...
    def peak_weight_center(self, center, span):
        ...
        sigma = (np.square(np.arange(win_span)-avg)
                 ).dot(ana_dos)/np.sum(ana_dos)
        sigma = np.sqrt(sigma)
...
#
        ...
        for atomi in range(self.atom.natom):
            for i in range(3):
                for m in range(3):
                    ...
                    A[atomi, i, m, :] = np.array(
                        Q_dagger[atomi]).dot(deltaG_im)
                    for j in range(3):
                        for k in range(3):
                            nye_tensor[atomi, j,
                                       k] -= self.perm_parity([j, i, m])*A[atomi, i, m, k]
            ...
            if (type(j) is int):
                ...
                if (anglemin > criteria):
                    if (indexk not in Qunsort.keys()):  # (Q[ref][indexk]==[]):
                        ...
                        Qunsort[indexk] = [neighbor[j][DIST],
                                           neighbor[j][DX], anglemin, j]
                        del neighbor[j]
                        ...
                        neighbor[Qunsort[indexk][ID]] = [
                            Qunsort[indexk][DIST], Qunsort[indexk][DX]]
                        Qunsort[indexk] = [neighbor[j][DIST],
                                           neighbor[j][DX], anglemin, j]
                        del neighbor[j]
        ...
        if ((P_allspecies is not None) and (Pn_allspecies is not None)):
            ...
            for i in range(self.atom.natom):
                ...
                if (Pn is None):
                    ...
                    return None, None, None
                least_missing_id = -1
                least_missing_value = np.max(
                    [len(Pn[ref][:, 0]) for ref in range(len(Pn))])
                angle_store = []
                Qunsort = []
                ...
                for ref in range(len(Pn)):
                    ...
                    for j in ngh_list[i].keys():
                        for k in range(len(Pn[ref])):
                            angle_store[ref][j, k] = np.array(ngh_list[i][j][DX]).dot(
                                Pn[ref][k, :])/ngh_list[i][j][DIST]
                    self.match_neigh2P(
                        ngh_list[i], Pn[ref], angle_store[ref], Qunsort[ref])
                ...
                if (change_P == False):
                    ...
                    while ((missing_ngh > 0) and (criteria > 0)):
                        self.match_neigh2P(
                            ngh_list[i], Pn[ref], angle_store[ref], Qunsort[ref], criteria, criteria+0.1)
                        missing_ngh = len(Pn[ref])-len(Qunsort[ref])
                        criteria -= 0.05
                    if (missing_ngh > len(ngh_list[i])):
                        ...
                        self.add_nearest_neigh(
                            ngh_list[i], Pn[ref], angle_store[ref], Qunsort[ref], missing_ngh)
    ...
    def compute_strain_scalar(self, etaa=None, G_dagger=None):
        ...
        for i in range(l):
            ...
            G_temp = eta-np.array([[1, 0, 0], [0, 1, 0],
                                   [0, 0, 1]])*(volume[i])
            G_temp = G_temp.dot(G_temp)
            vonMises[i] = np.sqrt((G_temp[0, 0]+G_temp[1, 1]+G_temp[2, 2])/2.)
