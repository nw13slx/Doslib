...
class iodos:
    ...
    def read_poscar(self):
        ...
        if (self._posf):
            ...
            if ('s' == initial.lower()):
                starting = line_n+2
                a = np.array([line.strip().split()
                              for line in lines[starting:(starting+atom.natom)]])
                atom.positions = a[:, 0:3].astype(np.float)
                atom.constraints = a[:, 3:6]
            else:
                starting = line_n+1
                atom.positions = np.loadtxt(
                    lines[starting:(starting+atom.natom)])
    ...
    def write_pdosfile(self):
        ...
        dos = self.dos
        if dos.spin:
            matrix = np.hstack([dos.Xenergy.reshape(
                [dos.nedos, 1]), dos.perspecies[:, :, 0], dos.perspecies[:, :, 1]])
        else:
            matrix = np.hstack([dos.Xenergy.reshape(
                [dos.nedos, 1]), dos.perspecies[:, :, 0]])
    ...
    def write_peratom(self, atomi):
        ...
        np.savetxt(control.name+'DOS-atom'+str(atomi) +
                   '.gz', matrix, fmt='%15.8f')
        del matrix
    ...
    def read_pdos(self):
        ...
        for i in range(4):
            ...
            if par_element[i] >= atom.ntype:
                self.end = "the type # for orbital %s is higher then the total types of element" % par_symbol[
                    i]
        self._auto_terminate()
        ...
        for atomi in xrange(atom.natom):
            ...
            for orbital in range(4):
                if (species[atomi] == par_element[orbital]):
                    dos.par_orbital[orbital] += partial[:,
                                                        nspin*orbital:nspin*orbital+nspin]
...
#      #  np.savetxt(name+'DOS'+str(atomi)+".dat",matrix,fmt='%15.8f',header=header_line)
    ...
    def tally_nospin(self, element):
        ...
        if (ncols > 9):  # d
            partial[:, 2] = element[:, 5] + element[:, 6] + \
                element[:, 7] + element[:, 8] + element[:, 9]
            partial[:, 3] = element[:, 5] + \
                element[:, 6] + element[:, 8]  # dt2g
            partial[:, 4] = element[:, 7] + element[:, 9]  # deg
        if (ncols > 16):  # f
            partial[:, 5] = element[:, 10] + element[:, 11] + element[:, 12] + \
    ...
    def tally_spin(self, element):
        ...
        if (ncols > 18):  # d
            partial[:, 4] = element[:, 9] + element[:, 11] + \
                element[:, 13] + element[:, 15] + element[:, 17]
            partial[:, 5] -= element[:, 10] + element[:, 12] + \
                element[:, 14] + element[:, 16] + element[:, 18]
            partial[:, 8] = element[:, 9] + \
                element[:, 11] + element[:, 15]  # dt2g
            partial[:, 9] -= element[:, 10] + \
                element[:, 12] + element[:, 16]  # dt2g
            partial[:, 10] = element[:, 13] + element[:, 17]  # deg
            partial[:, 11] -= element[:, 14] + element[:, 18]  # deg
        if (ncols > 32):  # f
            partial[:, 6] = element[:, 19] + element[:, 21] + element[:, 23] + \
                element[:, 25] + element[:, 27]+element[:, 29]+element[:, 31]
            partial[:, 7] -= (element[:, 20] + element[:, 22] + element[:, 24] +
                              element[:, 26] + element[:, 28]+element[:, 30]+element[:, 32])
        return tot, partial
